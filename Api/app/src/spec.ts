...
export const ROOT_ROUTES: RootRoute[] = [
  ...
  { method: 'post', path: '/sysload', summary: 'Host load overview (cpu/mem/disk/swap/uptime), unprivileged. The dayz unit footprint moved to the "status" action, which already takes that snapshot — this endpoint crosses no privilege boundary.', auth: true,
    response: S.obj({
      uptimeSec: S.int,
