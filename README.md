...
Everything on **servermander.ovh** that is served *behind nginx* lives in this
folder. nginx + TLS is a **shared requirement**, so it is the root; the services
that depend on it are nested inside. The folder tree *is* the dependency graph:
...
Code utilities (`Get-Stdout`, `Write-CsvLog`) live at `Dev/common/Utils.ps1`,
one level **above** this repo. This folder is a single git repo rooted **here**
(branch `main`; DayZ Server is deliberately *not* in it), but every deploy
script dot-sources that shared util from outside the repo — so the repo assumes
it stays in place at `Dev/UbuntuHost/NginxService/` with `Dev/common/`
alongside. That is fine in practice: these scripts only ever run from the dev
machine (they rsync/ssh to the box — the box never clones this repo), so the
path always resolves. A standalone clone *elsewhere* would need `Utils.ps1`
copied in.
...
The host base is done automatically by whichever service you provision **first**,
so no service depends on another having run. Certificates last 90 days and
...
The two services share **nothing** but the nginx daemon itself — different certs,
different vhost files, different payload directories on the box
(`/var/www/personal-projects` vs `/opt/cryptpad` + `/var/lib/cryptpad`). You can
...
built as a string inside the script, and nothing lands on the box that you can't
...
There is exactly one unavoidable shared fact: **there is a single nginx daemon.**
Any `nginx -t` / reload validates *all* installed vhosts at once, so a broken
vhost in one service can block a reload for the other. That is nginx being nginx —
it cannot be split.

> **Open item:** the `default_server` → 404 catch-all (what happens to subdomains
> no service claims) currently lives *inside StaticishSite's vhost*, which quietly
> makes CryptPad's routing depend on StaticishSite. Its proper home is a
> host-level catch-all owned by this `NginxService/` layer, belonging to neither
> service. Not yet lifted.
