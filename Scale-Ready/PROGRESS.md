...
| E6 | 2 | B | **"View default" shows the default SIDE BY SIDE (owner 2026-07-31, verbatim in PLAN.md E6)** | plan | - | ConfigViewer | TODO — today `wfShowDefault` REPLACES the live text (`editor.js:846`, `:1002`), so nothing can be compared. The comparison already exists in own-editor (CM6 `unifiedMergeView` vs the frozen `.defaults`) — reuse it, do not hand-roll a second diff. Needs a design proof (UI) |
