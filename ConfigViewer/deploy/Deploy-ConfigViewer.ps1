...
$rsyncArgs = @(
    ...
    # web/tiles/ is generated by the MapDataExtraction project and gitignored (~323 MB),
    # so a fresh checkout has no local copy. Protect it from --delete: local tiles still
    # upload when present, but the server's tiles are never removed by a deploy. To
    # deliberately drop a map's tiles, delete them on the box over ssh.
    '--filter=P /tiles/***',
    '-e', $sshCmd,
    ("{0}/" -f $webDir),
    $remote
